...
class Docker implements Serializable {
    ...
    def deleteUntaggedImage(String repositoryName) {
        script.sh "aws ecr batch-delete-image --repository-name ${repositoryName} --image-ids \"\$(aws ecr describe-images --repository-name ${repositoryName} --query 'imageDetails[?imageTags==null].[{\"imageDigest\": imageDigest}]' --output json | tr -d '[]')\""
    }
